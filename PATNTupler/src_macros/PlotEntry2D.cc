...
void PlotEntry2D::AddInput2018squark(const std::string& flatTreeAddress, const std::string& selectionCut, const double& crossSection, const std::string& scaleFactorWeightStr)
{
	TFile * f = TFile::Open(flatTreeAddress.c_str());
    TTree * evT = (TTree*)f->Get("eventCountTree");
    Int_t evTEntries = (Int_t)evT->GetEntries();
	...
	double ncor = T->GetEntries("nGluino==0")*nEvtsRunOverForInputTotal/double(numberOfEventsAfterPreSelection);

	if (T->GetEntries("nGluino==0") != numberOfEventsAfterPreSelection){
		std::cout << "number of events " << T->GetEntries("nGluino==0") << " in doubleBFatJetPairTree of " << flatTreeAddress << " does not equal those in the nEvtsPass count " << numberOfEventsAfterPreSelection << std::endl;
		std::cout << "number of events in doubleBFatJetPairTree " << T->GetEntries("nGluino==0") << " correction factor: " << nEvtsRunOverForInputTotal/numberOfEventsAfterPreSelection << " corrected: " << ncor << std::endl;
		// return;
	}

    double eventWeighting = 1000.0 * crossSection * luminosity / ncor;
...
void PlotEntry2D::AddInputFromGrid(const std::string& flatTreeAddress, const std::string& selectionCut, const double& crossSection, const std::string& scaleFactorWeightStr)
{
	TFile * f = TFile::Open(flatTreeAddress.c_str());
    TTree * evT = (TTree*)f->Get("eventCountTree");
    Int_t evTEntries = (Int_t)evT->GetEntries();
