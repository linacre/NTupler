...
int main(){
    ...
    const std::string outputDir = "./histos_plot_2018_newvalidationregions/"; // where we are going to save the output plots (should include the samples name, and any important features)
    ...
    std::vector<TH1D*> indiHistoVec = {h16_["S_alternative_databkgsub"], h16_["predNew_alternative_databkgsub"]};
/*
...
*/
    ...
    std::vector<std::string> legendNames = {"alternative", "alternative prediction"};
    ...
    std::string plotName = "linear_alternative_2018";
    plot.SaveSpec01(Form("%s/%s.pdf", outputDir.c_str(), plotName.c_str()), stringVec);
    
    plotName = "log_alternative_2018";
    plot.SetLogY();
...
void GetHistograms(std::map<std::string,TH1D*>& h_, int year)
{
    ...
    if(year==2018) {
        ...
        preamble = "/home/ppd/xxt18833/NMSSM/PATNTupler_18/macros/2018_histos_ht_extraValidation/";
    ...
    */
    ...
    for (size_t iH = 0; iH < histoNameVec.size(); ++iH){
        ...
        h_[Form("S_down_%s", histoToUse.c_str())] = (TH1D*)f->Get("S_dbtDiagDownLoose_NOSYS");
        h_[Form("U_down_%s", histoToUse.c_str())] = (TH1D*)f->Get("U_dbtDiagDownLoose_NOSYS");
        h_[Form("D_down_%s", histoToUse.c_str())] = (TH1D*)f->Get("D_dbtDiagDownLoose_NOSYS");
        ...
        h_[Form("S_alternative_%s", histoToUse.c_str())] = (TH1D*)f->Get("S_dbtLooseMaxAndIDBTCv23Loose_NOSYS");
        h_[Form("U_alternative_%s", histoToUse.c_str())] = (TH1D*)f->Get("U_dbtLooseMaxAndIDBTCv23Loose_NOSYS");
        h_[Form("D_alternative_%s", histoToUse.c_str())] = (TH1D*)f->Get("D_dbtLooseMaxAndIDBTCv23Loose_NOSYS");
        h_[Form("S_alternative_%s", histoToUse.c_str())]->Add((TH1D*)f->Get("S_dbtIDBTCv23LooseAndLooseMax_NOSYS"));
        h_[Form("U_alternative_%s", histoToUse.c_str())]->Add((TH1D*)f->Get("U_dbtIDBTCv23LooseAndLooseMax_NOSYS"));
        h_[Form("D_alternative_%s", histoToUse.c_str())]->Add((TH1D*)f->Get("D_dbtIDBTCv23LooseAndLooseMax_NOSYS"));
        ...
        h_[Form("UnD_down_%s", histoToUse.c_str())] = (TH1D*)h_[Form("U_down_%s", histoToUse.c_str())]->Clone();
        h_[Form("UnD_down_%s", histoToUse.c_str())]->Add(h_[Form("D_down_%s", histoToUse.c_str())]);
        ...
        h_[Form("UnD_alternative_%s", histoToUse.c_str())] = (TH1D*)h_[Form("U_alternative_%s", histoToUse.c_str())]->Clone();
        h_[Form("UnD_alternative_%s", histoToUse.c_str())]->Add(h_[Form("D_alternative_%s", histoToUse.c_str())]);
        ...
        if(histoToUse == "data") {
                ...
                h_[Form("S_down_%sbkgsub", histoToUse.c_str())] = (TH1D*)h_[Form("S_down_%s", histoToUse.c_str())]->Clone();
                h_[Form("UnD_down_%sbkgsub", histoToUse.c_str())] = (TH1D*)h_[Form("UnD_down_%s", histoToUse.c_str())]->Clone();
                ...
                h_[Form("S_alternative_%sbkgsub", histoToUse.c_str())] = (TH1D*)h_[Form("S_alternative_%s", histoToUse.c_str())]->Clone();
                h_[Form("UnD_alternative_%sbkgsub", histoToUse.c_str())] = (TH1D*)h_[Form("UnD_alternative_%s", histoToUse.c_str())]->Clone();
            ...
            for (size_t iB = 0; iB < bgNameVec.size(); ++iB){
                ...
                h_[Form("S_down_%sbkgsub", histoToUse.c_str())]->Add( h_[Form("S_down_%s", bgToUse.c_str())], -1);
                h_[Form("UnD_down_%sbkgsub", histoToUse.c_str())]->Add( h_[Form("UnD_down_%s", bgToUse.c_str())], -1);
                ...
                h_[Form("S_alternative_%sbkgsub", histoToUse.c_str())]->Add( h_[Form("S_alternative_%s", bgToUse.c_str())], -1);
                h_[Form("UnD_alternative_%sbkgsub", histoToUse.c_str())]->Add( h_[Form("UnD_alternative_%s", bgToUse.c_str())], -1);
            }
        }


        h_[Form("S_controlsum_%s", histoToUse.c_str())] = (TH1D*)h_[Form("S_control_%s", histoToUse.c_str())]->Clone();
        h_[Form("S_controlsum_%s", histoToUse.c_str())]->Add(h_[Form("S_alternative_%s", histoToUse.c_str())]);
        h_[Form("S_controlsum_%s", histoToUse.c_str())]->Add(h_[Form("S_down_%s", histoToUse.c_str())]);

        h_[Form("UnD_controlsum_%s", histoToUse.c_str())] = (TH1D*)h_[Form("UnD_control_%s", histoToUse.c_str())]->Clone();
        h_[Form("UnD_controlsum_%s", histoToUse.c_str())]->Add(h_[Form("UnD_alternative_%s", histoToUse.c_str())]);
        h_[Form("UnD_controlsum_%s", histoToUse.c_str())]->Add(h_[Form("UnD_down_%s", histoToUse.c_str())]);

        if(histoToUse == "data") {
            h_[Form("S_controlsum_%sbkgsub", histoToUse.c_str())] = (TH1D*)h_[Form("S_control_%sbkgsub", histoToUse.c_str())]->Clone();
            h_[Form("S_controlsum_%sbkgsub", histoToUse.c_str())]->Add(h_[Form("S_alternative_%sbkgsub", histoToUse.c_str())]);
            h_[Form("S_controlsum_%sbkgsub", histoToUse.c_str())]->Add(h_[Form("S_down_%sbkgsub", histoToUse.c_str())]);

            h_[Form("UnD_controlsum_%sbkgsub", histoToUse.c_str())] = (TH1D*)h_[Form("UnD_control_%sbkgsub", histoToUse.c_str())]->Clone();
            h_[Form("UnD_controlsum_%sbkgsub", histoToUse.c_str())]->Add(h_[Form("UnD_alternative_%sbkgsub", histoToUse.c_str())]);
            h_[Form("UnD_controlsum_%sbkgsub", histoToUse.c_str())]->Add(h_[Form("UnD_down_%sbkgsub", histoToUse.c_str())]);
        }


        // NEW METHOD OF PREDICTION

        h_[Form("predNew_anti_%s", histoToUse.c_str())] = (TH1D*)h_[Form("UnD_anti_%s", histoToUse.c_str())]->Clone();
        if(histoToUse == "data") h_[Form("predNew_anti_%sbkgsub", histoToUse.c_str())] = (TH1D*)h_[Form("UnD_anti_%sbkgsub", histoToUse.c_str())]->Clone();
        for (int iBin = 1; iBin < h_[Form("predNew_anti_%s", histoToUse.c_str())]->GetNbinsX() + 1; ++iBin){
            double corrValue = QcdSidebandCorr::GetCorr(iBin, year);
            double corrError = QcdSidebandCorr::GetCorrErr(iBin, year);
            double UnDValue = h_[Form("predNew_anti_%s", histoToUse.c_str())]->GetBinContent(iBin);
            double UnDError = h_[Form("predNew_anti_%s", histoToUse.c_str())]->GetBinError(iBin);
            ...
            h_[Form("predNew_anti_%s", histoToUse.c_str())]->SetBinContent(iBin, predValue);
            h_[Form("predNew_anti_%s", histoToUse.c_str())]->SetBinError(iBin, predError);

            if(histoToUse == "data") {
                UnDValue = h_[Form("predNew_anti_%sbkgsub", histoToUse.c_str())]->GetBinContent(iBin);
                UnDError = h_[Form("predNew_anti_%sbkgsub", histoToUse.c_str())]->GetBinError(iBin);

                predValue = corrValue * UnDValue;
                predError = 0.0;
                if (UnDValue != 0) predError = predValue * sqrt( (corrError/corrValue)*(corrError/corrValue) + (UnDError/UnDValue)*(UnDError/UnDValue) );
                h_[Form("predNew_anti_%sbkgsub", histoToUse.c_str())]->SetBinContent(iBin, predValue);
                h_[Form("predNew_anti_%sbkgsub", histoToUse.c_str())]->SetBinError(iBin, predError);

            }
        }

        h_[Form("predNew_tag_%s", histoToUse.c_str())] = (TH1D*)h_[Form("UnD_tag_%s", histoToUse.c_str())]->Clone();
        if(histoToUse == "data") h_[Form("predNew_tag_%sbkgsub", histoToUse.c_str())] = (TH1D*)h_[Form("UnD_tag_%sbkgsub", histoToUse.c_str())]->Clone();
        for (int iBin = 1; iBin < h_[Form("predNew_tag_%s", histoToUse.c_str())]->GetNbinsX() + 1; ++iBin){
            ...
            if(histoToUse == "data") {
                UnDValue = h_[Form("predNew_tag_%sbkgsub", histoToUse.c_str())]->GetBinContent(iBin);
                UnDError = h_[Form("predNew_tag_%sbkgsub", histoToUse.c_str())]->GetBinError(iBin);
        ...
        h_[Form("predNew_down_%s", histoToUse.c_str())] = (TH1D*)h_[Form("UnD_down_%s", histoToUse.c_str())]->Clone();
        if(histoToUse == "data") h_[Form("predNew_down_%sbkgsub", histoToUse.c_str())] = (TH1D*)h_[Form("UnD_down_%sbkgsub", histoToUse.c_str())]->Clone();
        for (int iBin = 1; iBin < h_[Form("predNew_down_%s", histoToUse.c_str())]->GetNbinsX() + 1; ++iBin){
            double corrValue = QcdSidebandCorr::GetCorr(iBin, year);
            double corrError = QcdSidebandCorr::GetCorrErr(iBin, year);
            double UnDValue = h_[Form("predNew_down_%s", histoToUse.c_str())]->GetBinContent(iBin);
            double UnDError = h_[Form("predNew_down_%s", histoToUse.c_str())]->GetBinError(iBin);

            double predValue = corrValue * UnDValue;
            double predError = 0.0;
            if (UnDValue != 0) predError = predValue * sqrt( (corrError/corrValue)*(corrError/corrValue) + (UnDError/UnDValue)*(UnDError/UnDValue) );
            h_[Form("predNew_down_%s", histoToUse.c_str())]->SetBinContent(iBin, predValue);
            h_[Form("predNew_down_%s", histoToUse.c_str())]->SetBinError(iBin, predError);

            if(histoToUse == "data") {
                UnDValue = h_[Form("predNew_down_%sbkgsub", histoToUse.c_str())]->GetBinContent(iBin);
                UnDError = h_[Form("predNew_down_%sbkgsub", histoToUse.c_str())]->GetBinError(iBin);

                predValue = corrValue * UnDValue;
                predError = 0.0;
                if (UnDValue != 0) predError = predValue * sqrt( (corrError/corrValue)*(corrError/corrValue) + (UnDError/UnDValue)*(UnDError/UnDValue) );
                h_[Form("predNew_down_%sbkgsub", histoToUse.c_str())]->SetBinContent(iBin, predValue);
                h_[Form("predNew_down_%sbkgsub", histoToUse.c_str())]->SetBinError(iBin, predError);

            }
        }

        h_[Form("predNew_control_%s", histoToUse.c_str())] = (TH1D*)h_[Form("UnD_control_%s", histoToUse.c_str())]->Clone();
        if(histoToUse == "data") h_[Form("predNew_control_%sbkgsub", histoToUse.c_str())] = (TH1D*)h_[Form("UnD_control_%sbkgsub", histoToUse.c_str())]->Clone();
        for (int iBin = 1; iBin < h_[Form("predNew_control_%s", histoToUse.c_str())]->GetNbinsX() + 1; ++iBin){
            ...
            if(histoToUse == "data") {
                UnDValue = h_[Form("predNew_control_%sbkgsub", histoToUse.c_str())]->GetBinContent(iBin);
                UnDError = h_[Form("predNew_control_%sbkgsub", histoToUse.c_str())]->GetBinError(iBin);
        ...
        h_[Form("predNew_alternative_%s", histoToUse.c_str())] = (TH1D*)h_[Form("UnD_alternative_%s", histoToUse.c_str())]->Clone();
        if(histoToUse == "data") h_[Form("predNew_alternative_%sbkgsub", histoToUse.c_str())] = (TH1D*)h_[Form("UnD_alternative_%sbkgsub", histoToUse.c_str())]->Clone();
        for (int iBin = 1; iBin < h_[Form("predNew_alternative_%s", histoToUse.c_str())]->GetNbinsX() + 1; ++iBin){
            double corrValue = QcdSidebandCorr::GetCorr(iBin, year);
            double corrError = QcdSidebandCorr::GetCorrErr(iBin, year);
            double UnDValue = h_[Form("predNew_alternative_%s", histoToUse.c_str())]->GetBinContent(iBin);
            double UnDError = h_[Form("predNew_alternative_%s", histoToUse.c_str())]->GetBinError(iBin);

            double predValue = corrValue * UnDValue;
            double predError = 0.0;
            if (UnDValue != 0) predError = predValue * sqrt( (corrError/corrValue)*(corrError/corrValue) + (UnDError/UnDValue)*(UnDError/UnDValue) );
            h_[Form("predNew_alternative_%s", histoToUse.c_str())]->SetBinContent(iBin, predValue);
            h_[Form("predNew_alternative_%s", histoToUse.c_str())]->SetBinError(iBin, predError);

            if(histoToUse == "data") {
                UnDValue = h_[Form("predNew_alternative_%sbkgsub", histoToUse.c_str())]->GetBinContent(iBin);
                UnDError = h_[Form("predNew_alternative_%sbkgsub", histoToUse.c_str())]->GetBinError(iBin);

                predValue = corrValue * UnDValue;
                predError = 0.0;
                if (UnDValue != 0) predError = predValue * sqrt( (corrError/corrValue)*(corrError/corrValue) + (UnDError/UnDValue)*(UnDError/UnDValue) );
                h_[Form("predNew_alternative_%sbkgsub", histoToUse.c_str())]->SetBinContent(iBin, predValue);
                h_[Form("predNew_alternative_%sbkgsub", histoToUse.c_str())]->SetBinError(iBin, predError);

            }
        }

        h_[Form("predNew_controlsum_%s", histoToUse.c_str())] = (TH1D*)h_[Form("UnD_controlsum_%s", histoToUse.c_str())]->Clone();
        if(histoToUse == "data") h_[Form("predNew_controlsum_%sbkgsub", histoToUse.c_str())] = (TH1D*)h_[Form("UnD_controlsum_%sbkgsub", histoToUse.c_str())]->Clone();
        for (int iBin = 1; iBin < h_[Form("predNew_controlsum_%s", histoToUse.c_str())]->GetNbinsX() + 1; ++iBin){
            double corrValue = QcdSidebandCorr::GetCorr(iBin, year);
            double corrError = QcdSidebandCorr::GetCorrErr(iBin, year);
            double UnDValue = h_[Form("predNew_controlsum_%s", histoToUse.c_str())]->GetBinContent(iBin);
            double UnDError = h_[Form("predNew_controlsum_%s", histoToUse.c_str())]->GetBinError(iBin);

            double predValue = corrValue * UnDValue;
            double predError = 0.0;
            if (UnDValue != 0) predError = predValue * sqrt( (corrError/corrValue)*(corrError/corrValue) + (UnDError/UnDValue)*(UnDError/UnDValue) );
            h_[Form("predNew_controlsum_%s", histoToUse.c_str())]->SetBinContent(iBin, predValue);
            h_[Form("predNew_controlsum_%s", histoToUse.c_str())]->SetBinError(iBin, predError);

            if(histoToUse == "data") {
                UnDValue = h_[Form("predNew_controlsum_%sbkgsub", histoToUse.c_str())]->GetBinContent(iBin);
                UnDError = h_[Form("predNew_controlsum_%sbkgsub", histoToUse.c_str())]->GetBinError(iBin);

                predValue = corrValue * UnDValue;
                predError = 0.0;
                if (UnDValue != 0) predError = predValue * sqrt( (corrError/corrValue)*(corrError/corrValue) + (UnDError/UnDValue)*(UnDError/UnDValue) );
                h_[Form("predNew_controlsum_%sbkgsub", histoToUse.c_str())]->SetBinContent(iBin, predValue);
                h_[Form("predNew_controlsum_%sbkgsub", histoToUse.c_str())]->SetBinError(iBin, predError);

            }
        }

        // OLD METHOD OF PREDICTION (note, no bkgsub implemented)
        h_[Form("predOld_anti_%s", histoToUse.c_str())] = (TH1D*)h_[Form("UnD_anti_%s", histoToUse.c_str())]->Clone();
        h_[Form("predOld_anti_%s", histoToUse.c_str())]->Multiply(h_[Form("S_anti_%s", histoToUse.c_str())]);
        h_[Form("predOld_anti_%s", histoToUse.c_str())]->Divide(h_[Form("UnD_anti_%s", histoToUse.c_str())]);
        ...
        h_[Form("predOld_down_%s", histoToUse.c_str())] = (TH1D*)h_[Form("UnD_down_%s", histoToUse.c_str())]->Clone();
        h_[Form("predOld_down_%s", histoToUse.c_str())]->Multiply(h_[Form("S_anti_%s", histoToUse.c_str())]);
        h_[Form("predOld_down_%s", histoToUse.c_str())]->Divide(h_[Form("UnD_anti_%s", histoToUse.c_str())]);
        ...
        h_[Form("predOld_alternative_%s", histoToUse.c_str())] = (TH1D*)h_[Form("UnD_alternative_%s", histoToUse.c_str())]->Clone();
        h_[Form("predOld_alternative_%s", histoToUse.c_str())]->Multiply(h_[Form("S_anti_%s", histoToUse.c_str())]);
        h_[Form("predOld_alternative_%s", histoToUse.c_str())]->Divide(h_[Form("UnD_anti_%s", histoToUse.c_str())]);

        h_[Form("predOld_controlsum_%s", histoToUse.c_str())] = (TH1D*)h_[Form("UnD_controlsum_%s", histoToUse.c_str())]->Clone();
        h_[Form("predOld_controlsum_%s", histoToUse.c_str())]->Multiply(h_[Form("S_anti_%s", histoToUse.c_str())]);
        h_[Form("predOld_controlsum_%s", histoToUse.c_str())]->Divide(h_[Form("UnD_anti_%s", histoToUse.c_str())]);

        // now repeat with bkgsub
        if(histoToUse == "data") {
            h_[Form("predOld_anti_%sbkgsub", histoToUse.c_str())] = (TH1D*)h_[Form("UnD_anti_%sbkgsub", histoToUse.c_str())]->Clone();
            h_[Form("predOld_anti_%sbkgsub", histoToUse.c_str())]->Multiply(h_[Form("S_anti_%sbkgsub", histoToUse.c_str())]);
            h_[Form("predOld_anti_%sbkgsub", histoToUse.c_str())]->Divide(h_[Form("UnD_anti_%sbkgsub", histoToUse.c_str())]);

            h_[Form("predOld_tag_%sbkgsub", histoToUse.c_str())] = (TH1D*)h_[Form("UnD_tag_%sbkgsub", histoToUse.c_str())]->Clone();
            h_[Form("predOld_tag_%sbkgsub", histoToUse.c_str())]->Multiply(h_[Form("S_anti_%sbkgsub", histoToUse.c_str())]);
            h_[Form("predOld_tag_%sbkgsub", histoToUse.c_str())]->Divide(h_[Form("UnD_anti_%sbkgsub", histoToUse.c_str())]);

            h_[Form("predOld_down_%sbkgsub", histoToUse.c_str())] = (TH1D*)h_[Form("UnD_down_%sbkgsub", histoToUse.c_str())]->Clone();
            h_[Form("predOld_down_%sbkgsub", histoToUse.c_str())]->Multiply(h_[Form("S_anti_%sbkgsub", histoToUse.c_str())]);
            h_[Form("predOld_down_%sbkgsub", histoToUse.c_str())]->Divide(h_[Form("UnD_anti_%sbkgsub", histoToUse.c_str())]);

            h_[Form("predOld_control_%sbkgsub", histoToUse.c_str())] = (TH1D*)h_[Form("UnD_control_%sbkgsub", histoToUse.c_str())]->Clone();
            h_[Form("predOld_control_%sbkgsub", histoToUse.c_str())]->Multiply(h_[Form("S_anti_%sbkgsub", histoToUse.c_str())]);
            h_[Form("predOld_control_%sbkgsub", histoToUse.c_str())]->Divide(h_[Form("UnD_anti_%sbkgsub", histoToUse.c_str())]);

            h_[Form("predOld_alternative_%sbkgsub", histoToUse.c_str())] = (TH1D*)h_[Form("UnD_alternative_%sbkgsub", histoToUse.c_str())]->Clone();
            h_[Form("predOld_alternative_%sbkgsub", histoToUse.c_str())]->Multiply(h_[Form("S_anti_%sbkgsub", histoToUse.c_str())]);
            h_[Form("predOld_alternative_%sbkgsub", histoToUse.c_str())]->Divide(h_[Form("UnD_anti_%sbkgsub", histoToUse.c_str())]);

            h_[Form("predOld_controlsum_%sbkgsub", histoToUse.c_str())] = (TH1D*)h_[Form("UnD_controlsum_%sbkgsub", histoToUse.c_str())]->Clone();
            h_[Form("predOld_controlsum_%sbkgsub", histoToUse.c_str())]->Multiply(h_[Form("S_anti_%sbkgsub", histoToUse.c_str())]);
            h_[Form("predOld_controlsum_%sbkgsub", histoToUse.c_str())]->Divide(h_[Form("UnD_anti_%sbkgsub", histoToUse.c_str())]);
        }
