...
int main(int argc, char** argv){
    ...
    std::vector<std::vector<std::string>> cut2_ak8DbtB = { {"Off","Max","Off","Max"} };
    std::vector<std::vector<std::string>> cut2_ak8DbtM = { {"DIAG_UP", "Loose"} };
    std::vector<std::vector<std::string>> cut2_ak8Dbt = doB ? cut2_ak8DbtB : cut2_ak8DbtM; // 4 elements in sub-vector: 1st for fatJetA min, 2nd for fatJetA max, 3rd for fatJetB min, 4th for fatJetB max --> "Off", "Loose", "Med1", "Med2", "Tight", "Max"
                ...
                for (size_t iCut5 = 0; iCut5 < cut5_ak4Pt.size(); ++iCut5){
                    ...
                    if (doB) {
                        ...
                        for (size_t iMassRegion = 0; iMassRegion!=numberOfSegments; ++iMassRegion){
                            AllMassRegionCuts += "( " + MassCutsObject.Get_C_Cuts()[iMassRegion] + " )";
                            if(iMassRegion != numberOfSegments - 1) AllMassRegionCuts += " || ";
                            else AllMassRegionCuts += " )";
                        }
                    }
                    ...
                    std::string cutToApply = Form("%s && fatJetA_p4.Pt()>%d && fatJetB_p4.Pt()>%d && ht>=%d && ht<%d && slimJetA_p4.Pt()>%d && slimJetB_p4.Pt()>%d%s", dbtCut.c_str(), cut3_ak8Pt[iCut3], cut3_ak8Pt[iCut3], cut4_ht[iCut4][0], cut4_ht[iCut4][1], cut5_ak4Pt[iCut5][0], cut5_ak4Pt[iCut5][1], AllMassRegionCuts.c_str() );
                    ...
                    PlotEntry2D plot2dQCD = PlotEntry2D("Multijet", hTemplate, varToPlot.c_str(), luminosity);
                    if(doB) {
                        varToPlotSaveName = "Multijet" + varToPlotSaveName;
                        plot2dQCD.AddInput("/mercury/data2/linacre/Joe_backupfromScratch/xap79297/Analysis_boostedNmssmHiggs/flatTrees_2019_01_01/2017/QCD_HT1000to1500_ht1499plus/flatTree.root", cutToApply.c_str(), 1005, "weight_combined");
                        plot2dQCD.AddInput("/mercury/data2/linacre/Joe_backupfromScratch/xap79297/Analysis_boostedNmssmHiggs/flatTrees_2019_01_01/2017/QCD_HT1500to2000_ht1499plus/flatTree.root", cutToApply.c_str(), 101.8, "weight_combined");
...
*/
                    ...
                    if(doB) saveName += "_AllMassRegionCuts";
                    if(emptyGrid) {
                        saveName += "_Empty";
                        std::string outputFile = outputDir + "/" + saveName + ".pdf";
                        plot.Save2DEmpty(outputFile.c_str(), MassCutsObject);
                    }
                    else {
                        ...
                        std::string outputFile = outputDir + "/" + saveName + ".pdf";
                        if(doB) plot.Save2D(outputFile.c_str(), true);
                        else plot.Save2D(outputFile.c_str());
