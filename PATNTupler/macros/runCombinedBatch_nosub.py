...
for signalDir in signalDirs:
    ...
    g.write("cd %s\n" % (signalDir))
    g.write("mkdir d s b\n")
    g.write("combine -M FitDiagnostics --saveShapes --saveWithUncertainties --saveOverallShapes --preFitValue 0 --robustFit 1 --toysFrequentist -t -1 --expectSignal 0 --trackParameters rgx{ch[0-9]+_alpha} --mass %s --keyword-value %sB=%s --out b %s > b/FitDiagnostics.log\n" % (higgsMass, keyword, susyMass, fileToUseTXT) )
    g.write("combine -M FitDiagnostics --saveShapes --saveWithUncertainties --saveOverallShapes --preFitValue 0 --robustFit 1 --toysFrequentist -t -1 --expectSignal 1 --trackParameters rgx{ch[0-9]+_alpha} --mass %s --keyword-value %sS=%s --out s %s > s/FitDiagnostics.log\n" % (higgsMass, keyword, susyMass, fileToUseTXT) )
    g.write("combine -M FitDiagnostics --saveShapes --saveWithUncertainties --saveOverallShapes --preFitValue 0 --robustFit 1 --trackParameters rgx{ch[0-9]+_alpha} --mass %s --keyword-value %sD=%s --out d %s > d/FitDiagnostics.log\n" % (higgsMass, keyword, susyMass, fileToUseTXT) )
    g.write("python /opt/ppd/scratch-2021/xxt18833/CMSSW_10_2_12/src/HiggsAnalysis/CombinedLimit/test/diffNuisances.py -a b/fitDiagnostics.root -g b/plots.root > b/diffNuisances.log\n")
    g.write("python /opt/ppd/scratch-2021/xxt18833/CMSSW_10_2_12/src/HiggsAnalysis/CombinedLimit/test/diffNuisances.py -a d/fitDiagnostics.root -g d/plots.root > d/diffNuisances.log\n")
    g.write("python /opt/ppd/scratch-2021/xxt18833/CMSSW_10_2_12/src/HiggsAnalysis/CombinedLimit/test/diffNuisances.py -a s/fitDiagnostics.root -g s/plots.root > s/diffNuisances.log\n")
    g.write("cd ..\n")
    g.close()
