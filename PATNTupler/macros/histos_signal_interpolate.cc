...
int main(){
    ...
    std::system(Form("cp $CMSSW_BASE/src/NTupler/PATNTupler/macros/histos_signal_interpolate.cc %s/%s__histos_signal_interpolate.cc", outputDir.c_str(), TimeStamp::GetTimeStamp().c_str()));
    ...
    // std::vector<TH1D*> indiHistoVec = {h16_["S_tag_mH30_mSusy1600"], h16_["S_tag_mH40_mSusy1600"], h16_["S_tag_mH50_mSusy1600"], h16_["S_tag_mH70_mSusy1600"], h16_["S_tag_mH90_mSusy1600"], h16_["S_tag_mH110_mSusy1600"], h16_["S_tag_mH125_mSusy1600"]}; // for post-fit

    // h16_["S_tag_mH40_mSusy1600"]->Print("all");
    // h16_["S_tag_mH50_mSusy1600"]->Print("all");
    // h16_["S_tag_mH70_mSusy1600"]->Print("all");

    /*
    TH1D * htemp = new TH1D("htemp", "", 3*nMassBins, 0, 3*nMassBins);
    TH1D * htemp2 = new TH1D("htemp2", "", 3*nMassBins, 0, 3*nMassBins);
    TH1D * htemp0 = new TH1D("htemp0", "", 3*nMassBins, 0, 3*nMassBins);

    for (unsigned int iBin=1; iBin<=3*nMassBins; iBin++){
        std::string histname = Form("S_tag_mH%d_mSusy1600", (100 / nMassBins)*( (iBin-1) % nMassBins) + 40);
        if(h16_[histname]) {
            htemp->SetBinContent(iBin, h16_[histname]->GetBinContent(iBin) );
            if( (iBin - 1) % nMassBins > 0) htemp2->SetBinContent(iBin, h16_[histname]->GetBinContent(iBin - 1) );
            if( (iBin - 1) % nMassBins + 1 < nMassBins) htemp0->SetBinContent(iBin, h16_[histname]->GetBinContent(iBin + 1) );
        }
    }
    htemp0->Print("all");
    htemp->Print("all");
    ...
    for (unsigned int iBin=1; iBin<=3*nMassBins; iBin++){
        ...
        if(h16_[histname]) {
            ...
            if( massBin - 1 >= 1 && massBin - 1 <= nMassBins) htemp2->SetBinContent(iBin, h16_[histname]->GetBinContent(iBin - 1) );
            if( massBin + 1 >= 1 && massBin + 1 <= nMassBins) htemp0->SetBinContent(iBin, h16_[histname]->GetBinContent(iBin + 1) );
        }
    }
    ...
    */
    ...
    for (int offset=minOffset; offset<= -minOffset; offset++){
        ...
        int point = 0;
        // TH1D * hMassDep_temp = new TH1D(Form("hMassDep_temp%d",hNumber), "", 3*nMassBins, 0, 3*nMassBins);
        std::vector<TGraph*> GraphsHT{new TGraph(), new TGraph(), new TGraph()};
        for (unsigned int iBin=1; iBin<=3*nMassBins; iBin++){
            int mass = (100 / nMassBins)*( (iBin-1) % nMassBins) + lowestSignalMass;
            std::string histname = Form("S_tag_mH%d_mSusy1600", mass);
            if(h16_[histname]) {
                unsigned int massBin = (iBin - 1) % nMassBins + 1;
        ...
        // hMassDep.push_back(hMassDep_temp);
        Graphs.push_back(GraphsHT);
    }
    
    std::vector<int> newMasses = {60, 80, 100};
    for (unsigned int iMass=0; iMass<newMasses.size(); iMass++) {
        h16_[Form("S_tag_mH%d_mSusy1600", newMasses[iMass])] = new TH1D(Form("S_tag_mH%d_mSusy1600", newMasses[iMass]), "", 3*nMassBins, 0, 3*nMassBins);
    }

    TCanvas * c = new TCanvas("can","can",2400,800);
    c->Divide(1 - 2*minOffset,3);
    int cNum = 1;

    for (int iht=0; iht<3; iht++) {    
        for (int offset=minOffset; offset<= -minOffset; offset++){
            int hNumber = offset-minOffset;
            // hMassDep[hNumber]->Print("all");

            Graphs[hNumber][iht]->Print();
            c->cd(cNum);
            Graphs[hNumber][iht]->Draw();
            // std::cout<<Graphs[hNumber][iht]->GetN()<<std::endl;
            int npoints = Graphs[hNumber][iht]->GetN();
            Graphs[hNumber][iht]->Fit(npoints > 2 ? "pol2" : "pol1");
