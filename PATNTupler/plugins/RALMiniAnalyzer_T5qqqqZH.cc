...
#include "NTupler/PATNTupler/interface/parse.h"
...
class RALMiniAnalyzer : public edm::EDAnalyzer {
   ...
   private:
      ...
      //parse LHE for SUSY  https://github.com/TreeMaker/TreeMaker/blob/Run2_2017/Utils/src/SignalScanProducer.cc#L144
    ...
    //cuts_(iConfig)
{
    ...
    EventDataTree->Branch("numHiggses", &numHiggses_, "numHiggses/I");
    EventDataTree->Branch("motherMass", &motherMass_, "motherMass/F");
    EventDataTree->Branch("lspMass", &lspMass_, "lspMass/F");
...
/*
     //Read in SUSY comment
     bool isGrid = true;
     if (isMC_ && isGrid) {
      std::cout<<"Attempting to read in SUSY comment (event)..."<<std::endl;
      ...
      if(!handles.empty()){
        ...
        getSUSYComment(*product);
        std::cout<<"Read in SUSY comment."<<std::endl;
      }
...
void 
RALMiniAnalyzer::beginLuminosityBlock(edm::LuminosityBlock const& iLumi, edm::EventSetup const& iSetup)
{
  // reset
  motherMass_ = -1;
  lspMass_ = -1;
  ...
  bool isGrid = true;
  if (isMC_ && isGrid) {
    //std::cout<<"Attempting to read in SUSY comment (lumi)..."<<std::endl;

    edm::Handle<GenLumiInfoHeader> gen_header;
    iLumi.getByToken(genLumiHeaderToken_, gen_header);
    getSUSYComment(*gen_header);
    //std::cout<<"Read in SUSY comment (lumi)."<<std::endl;
  }
...
/*
//parse LHE for SUSY
void RALMiniAnalyzer::getSUSYComment(const LHEEventProduct& lhe){
...
void RALMiniAnalyzer::getSUSYComment(const GenLumiInfoHeader& gen){
	getSUSYModelInfo(gen.configDescription());
}

//parse model comment for SUSY
void RALMiniAnalyzer::getSUSYModelInfo(std::string comment){
	//strip newline
	if(comment.back()=='\n') comment.pop_back();
	...
	std::cout<<" SUSY parameters: "<<lspMass<<" "<<motherMass<<std::endl;
  
  motherMass_ = motherMass;
	lspMass_ = lspMass;
}
...
void RALMiniAnalyzer::ReadInIsrInfo(const edm::Event& iEvent)
{
    ...
    nHiggs2bb = 0;
    int numHiggses=0;
    int numHiggses_test=0;
    for (const reco::GenParticle &iGenParticle: *genParticles){
        if( iGenParticle.pdgId() == 25 ) numHiggses_test++;
        if( iGenParticle.pdgId() == 25 &&
            iGenParticle.mother()->pdgId() == 1000023 &&
            iGenParticle.status() == 22 ) {
              numHiggses++;
              if ( iGenParticle.numberOfDaughters()==2 && abs(iGenParticle.daughter(0)->pdgId()) == 5  && abs(iGenParticle.daughter(1)->pdgId()) == 5 )
              {
                nHiggs2bb++; // Number of Higgs which decay to bb (should be all in Joe's samples, 58 percent in Alex's)
              }
            }
    ...
    //std::cout << "nHiggs2bb:" << nHiggs2bb << " numHiggses:" << numHiggses << " numHiggses_test-numHiggses:" << numHiggses_test-numHiggses << std::endl;

    nGLUINO_ = nGluino;
    nHIGGS2BB_ = nHiggs2bb;
    numHiggses_ = numHiggses;
}
